...
import { getSocialKPI } from "./svc_social"; // ✅ matches svc_social.ts export
...
async function buildApp() {
  ...
  app.get("/kpi/social", async (req, reply) => {
    try {
      ...
      // ✅ Always default to META_ACCESS_TOKEN if accessToken not passed
      const accessToken =
        String(q.accessToken || "") || String(process.env.META_ACCESS_TOKEN || "");

      const postLimit = q.postLimit ? Number(q.postLimit) : 5;
      ...
      if (!accessToken) {
        return reply.code(400).send({
          ...
          detail: "Set META_ACCESS_TOKEN in environment or provide ?accessToken=",
        });
      }
